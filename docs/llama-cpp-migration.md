...
| 端点 | 行为 |
|------|------|
| `GET /api/adapters` | `{ items: [], current: null }` |
| `POST /api/load-adapter` | 501 + 文案 |
| `POST /api/classify` | 501 |

### v2 路线

- LoRA：PEFT → GGUF LoRA + `/api/load-adapter` + `disable_adapter`（旁白绕过人格）
...
| 模块 | 旧 | 新 |
|------|---|---|
| Python sidecar | [`minicpm-pet-bridge/server.py`](../minicpm-pet-bridge/server.py)（已 deprecated） | [`minicpm-sidecar/gateway/server.py`](../minicpm-sidecar/gateway/server.py) |
| Pet 状态推送 | `minicpm-pet-bridge/clawd_state.py` | [`minicpm-sidecar/gateway/clawd_state.py`](../minicpm-sidecar/gateway/clawd_state.py)（原样） |
...
| 打包 | `extraResources` → `minicpm-pet-bridge-uv/` 源码子集 + adapters | 仅 `minicpm-sidecar/bin/<triple>/` + 瘦 source |
| 构建脚本 | `build/build-sidecar.sh`（torch 700MB） | [`minicpm-sidecar/scripts/build-all.sh`](../minicpm-sidecar/scripts/build-all.sh) |
...
| 路径 | 状态 |
|------|------|
| [`minicpm-pet-bridge/`](../minicpm-pet-bridge/) | DEPRECATED.md，保留作 v2 LoRA 移植参考 |
| [`minicpm-pet-bridge-uv/`](../minicpm-pet-bridge-uv/) | DEPRECATED.md（同上） |
| [`build/`](../build/) | DEPRECATED.md（旧 PyInstaller spec），新版在 `minicpm-sidecar/build/` |

下一个 minor 版本会整体删除这三个目录；现在保留只是方便 git history 与 v2 LoRA 工作。
